...
def make_shell_section(inp):
    """根据单元铺层数据生成CalculiX壳体截面复合层定义。

        参数:
        inp (tuple): (elem_id, plyarray) 单元ID和铺层数组
...
def material_db_to_ccx(grid, materials, matmap=None):
    """读取材料数据库并生成CalculiX材料属性块。

    参数:
        grid (str): 网格文件名
        materials (array): 材料ID数组
        matmap (str): 材料映射文件路径
...
def main():
    """主函数：将叶片VTU网格转换为CalculiX输入文件（.inp）。

        参数:
        通过命令行传入网格文件、输出文件名和材料映射文件"""
