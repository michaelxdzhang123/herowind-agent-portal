...
def join_up(nodes, epid, stacks, sec, poly):
    """
        ...
        中文说明:
        在铺层终止节点处使用Delaunay三角化连接相邻单元。

        参数:
        nodes (list): 需要连接的节点及相邻单元
        epid (list): 单元起始点ID列表
        stacks (list): 各单元铺层栈
        sec: 截面网格
        poly: 输出多边形数据"""
...
def get_node_avg_normal(connected_cell_normals):
    """根据相邻单元法向，计算节点的平均法向量，处理腹板连接情况。

        参数:
        connected_cell_normals (array): 相连单元法向量

        返回:
        tuple: (平均法向量, 被排除的法向量索引)"""
...
def cut_blade(r, vtu, if_bondline=True, rotz=0, var={}, is2d=False, verbose=False):
    """在指定半径处切割三维网格，生成二维截面网格。

        参数:
        r (float): 切割半径
        vtu (str): 输入网格文件
        if_bondline (bool): 是否创建胶接层
        rotz (float): 绕z轴旋转角度
        var (dict): 叶片变量字典
        is2d (bool): 输入是否为二维网格
    ...
    # Check if input mesh has points
    if msh.GetNumberOfPoints() == 0:
        print(f"Error: Input mesh {vtu} has no points.")
        return
    ...
    if not is2d:
        ...
        # Check if cutter produced any points
        if sec.GetNumberOfPoints() == 0:
            print(f"Warning: Cutter produced no points at radius {r}. The plane may not intersect the mesh.")
            return
    else:
        sec = msh

    # Check if section has cells
    if sec.GetNumberOfCells() == 0:
        print(f"Warning: Section at radius {r} has no cells. Skipping this section.")
        return
    ...
    # Check if the rotated section has any points
    if rotated_section.GetNumberOfPoints() == 0:
        print(f"Warning: No points found in rotated section at radius {r}. Skipping this section.")
        return
    ...
    # Check if section still has cells after cleaning
    if sec.GetNumberOfCells() == 0:
        print(f"Warning: Section at radius {r} has no cells after cleaning. Skipping this section.")
        return
    ...
    # Check if required cell data arrays exist
    if nply is None:
        print(f"Error: Required cell data array 'n_plies' not found in mesh at radius {r}.")
        return
    if thickness is None:
        print(f"Error: Required cell data array 'thickness' not found in mesh at radius {r}.")
        return
    if enormal is None:
        print(f"Error: Required cell data array 'Normals' not found in mesh at radius {r}.")
        return
    ...
    # Check if final output has points and cells
    if out.GetNumberOfPoints() == 0 or out.GetNumberOfCells() == 0:
        print(f"Warning: Final output mesh at radius {r} has no points or cells. Skipping output.")
        return
        ...
        try:
            ...
            for i in rr:
                try:
                    ...
                    print(f"Error processing radius {i}: {e2}")
    else:
        for i in rr:
            try:
                part(i)
            except Exception as e:
                print(f"Error processing radius {i}: {e}")


def main():
    """主函数：从三维FEA网格切片生成二维实体网格用于气动弹性梁属性计算。
