...
def compute_nodal_forces(nz, target_z, target_moment, fmult=1.0):
    """计算节点力分布，使给定截面处的弯矩与目标弯矩匹配。

        参数:
        nz (array): 节点z坐标数组
        target_z (array): 目标弯矩作用位置的z坐标
        target_moment (array): 目标弯矩值
        fmult (float): 力的方向乘子（1.0或-1.0）

        返回:
        tuple: (force, zpl, moment) 节点力数组、唯一z坐标列表和对应弯矩"""
    assert fmult ** 2 == 1
...
def main():
    """主函数：从YAML配置文件读取载荷信息，计算并施加节点力到网格上。

        参数:
        通过命令行参数传入yaml配置文件和网格文件路径"""
