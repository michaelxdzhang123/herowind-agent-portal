...
class section:
    ...
    def _create_evaluations(
        ...
        """创建用于重采样样条的评估点序列，考虑腹板分割点。

            参数:
            n_points (int): 总点数
            webs (list): 腹板列表
            mult (bool): 是否对首尾间隔加倍
            make_plots (bool): 是否绘图
    ...
    def respline(self, n_points, webs=[], added_datums={}, panel_mesh_scale=[]):
        """
            ...
            返回:
            tuple: (out, datums) 重采样后的点列表和坐标数据字典"""
