...
class blade:
    def __init__(
        ...
        """
            ...
            中文说明:
            初始化叶片对象，加载翼型、插值平面形状并生成截面。

            参数:
            chord (list): 弦长分布 [(r,chord),..]
            thickness (list): 相对厚度分布 [(r,t),..]
            twist (list): 扭角分布 [(r,twist),..]
            dx (list): x方向偏移分布
            dy (list): y方向偏移分布
            z (list): z坐标分布
            airfoils (dict): 翼型表（键为厚度值）
            np_spanwise (int): 展向点数
            chordwise_sampling (list): 弦向采样坐标列表（覆盖np_chordwise）
            offset_optimal (bool): 是否进行最优偏移
            interpolate_method (int): 插值类型（1线性，2 KochanekSpline等）
            flatten_lw (bool): 是否展平吸力面
            barrel_length (float): 圆柱形根部保留长度
            offset_clamp_points (list): 最优偏移参考点列表"""
    ...
    def _load_airfoils(self, airfoils, x=[]):
        """
            ...
            中文说明:
            加载翼型文件并通过插值填充self.airfoils。

            参数:
            airfoils (dict): 翼型字典（厚度值到文件名映射）
            x (list): 需要插值的弦向坐标列表"""
        self.airfoils = {}
    ...
    def _interpolate_planform(
        ...
        """
            ...
            参数:
            chord, thickness, twist, dx, dy, z: 各参数分布
            flatten_lw (bool): 是否展平吸力面"""
    ...
    def _interpolate_airfoils(
        ...
        """根据厚度分布插值翼型并生成截面序列。
            ...
            参数:
            interpolate_method (int): 插值方法类型
            offset_optimal (bool): 是否进行最优偏移
            offset_clamp_points (list): 偏移参考点"""
        v = []
    ...
    def export_variables(self, fname):
        """将叶片变量导出到文件并返回变量字典。
            ...
            返回:
            dict: 包含dx, dxf, dy, z, twist, chord等变量的字典"""
        var = {
            "dx": self.dx,
            "dxf": self.dxf,
            "dy": self.dy,
    ...
    def dump(self, fname="__sections.txt", z_rotation=0.0):
        """将截面列表导出为文件，用于有限元分析（含腹板）。

    参数:
        fname (str): 输出文件名（.txt或.pck）
